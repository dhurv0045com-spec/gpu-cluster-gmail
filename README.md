...
└─────────┼──────────────┼────────────┼────────────┼─────────┘
          ...
          │  ├── worker_A/                                    │
          │  │   ├── grad_step_000064.npz  ← sparse gradient  │
          │  │   └── grad_step_000128.npz                     │
          │  ├── worker_B/                                    │
          │  │   ├── grad_step_000064.npz  ← sparse gradient  │
          │  │   └── grad_step_000128.npz                     │
          │  └── worker_C/                                    │
...
2. Runs 64 forward/backward steps while accumulating a mean gradient
3. Saves the exact global top 1% by magnitude as a compressed fp16 `.npz` payload
4. POSTs `gradient_ready` with token count, model version, and real per-layer norms
5. Polls the mounted Drive state file for a versioned sparse delta
6. Applies the delta in place; a full checkpoint is loaded only for recovery
...
```bash
# Clone
git clone https://github.com/dhurv0045com-spec/gpu-cluster-gmail.git
cd gpu-cluster-gmail
...
2. Upload the common An-Ra base checkpoint into that folder
3. Share the `cluster` folder with each worker's Gmail (Editor role)
...
The first screen is **Connect Google Drive**. Complete OAuth with the primary
account that owns the cluster folder. Cluster configuration remains locked until
`/api/auth/status` confirms a valid credential.
...
{
  "step": 64,
  "grad_file_path": "/content/drive/MyDrive/AnRa/cluster/worker_A/grad_step_000064.npz",
  "minibatch_tokens": 65536,
  "model_version": 4,
  "layer_norms": [1.82, 1.47, 1.31]
}
...
1. In the *primary* account's Drive, right-click the `/AnRa/cluster/` folder → Share → add each additional Gmail → role: **Editor**, not Viewer.
2. Sign into [colab.research.google.com](https://colab.research.google.com) on the additional account specifically (check the profile photo in the top-right before running anything).
3. Open the worker notebook using the per-worker link generated by `SetupWizard.jsx`'s Step 3.
4. Mounting Drive on the additional account will surface the shared folder under "Shared with me" rather than "My Drive" — the pre-flight check in the worker notebook should detect and report which path is correct, rather than assuming `MyDrive`.

Colab does not directly mount ordinary “Shared with me” folders as a filesystem path.
Add a shortcut for the shared folder at `MyDrive/AnRa/cluster`; the notebook preflight
checks that path and the Shared drives path, then prints exact recovery guidance.

| Problem | Symptom | Fix |
|---|---|---|
| Folder shared as Viewer, not Editor | Worker can read but writes silently fail | Re-share with Editor role |
| Wrong Google account signed into Colab | `account_email` mismatch on registration | Sign out, sign in with correct Gmail |
| Drive free tier (15GB) nearly full from Gmail/Photos | Worker crashes with an I/O error mid-upload | Free up space on the *primary* account hosting the cluster folder |
| Colab idle/browser-closed timeout | Worker stops heartbeating with no error in the notebook | Keep the tab open; reconnect by rerunning from the registration cell — `worker_id` stays the same, so the coordinator treats it as the same worker resuming |
...
The parameter schema lets the model-free coordinator periodically fold deltas into a
recovery checkpoint without mistaking state-dict buffers for trainable parameters.
At 1% top-k, indices plus fp16 values are about **30 MB** for An-Ra's 499,167,047
parameters, before ZIP compression.
...
```python
indices, values = aggregate_sparse_gradients(worker_payloads)
# Values stay fp16 on Drive; learning-rate scaling happens in fp32 on receipt.
apply_sparse_delta_to_model(model, indices, values.float() * learning_rate)
```

Workers use Adafactor for lifecycle-compatible gradient clearing, while the shared
model update is a token-weighted sparse SGD delta. Keeping the update rule centralized
prevents optimizer-state drift between disconnected Colab runtimes.
...
- Each worker gradient and coordinator delta: roughly 30 MB at 1% sparsity
- Auto-cleanup keeps only the last 3 steps per worker
- A full recovery checkpoint is uploaded every 10 successful sync rounds, not every round
- `delta_history` records the exact chain after the latest recovery checkpoint
...
- The coordinator rejects stale submissions with HTTP 409 instead of mixing model versions
- The worker applies the missing delta chain (or recovery checkpoint) before computing again
...
The repaired protocol changes both sync frequency and payload shape:

| Quantity | Before | Repaired target |
|---|---:|---:|
| Sync frequency | every step | every 64 steps |
| Worker upload | ~1.0 GB dense fp16 | ~30 MB sparse |
| Worker download | ~2.0 GB checkpoint | ~30 MB sparse delta |
| Compute efficiency | ~0.8% | ≥85% target |
| 2-worker effective throughput | ~5.4 tok/s | ~650+ tok/s target |

Using the verified 2.9 s/step T4 compute time and a conservative 10 MB/s Drive
transfer rate, 64 steps provide 185.6 s of compute. Two 30 MB transfers plus six
seconds of merge/state overhead are about 12 s, or roughly **94% compute efficiency**
between recovery-checkpoint rounds. Production throughput still depends on the model,
Drive rate, Colab throttling, and coordinator CPU; the target is not presented as a
measurement until it is benchmarked on the live An-Ra deployment.
...
| `FRONTEND_URL` | `http://localhost:5173` | OAuth success redirect destination |
| `CHECKPOINT_EVERY_N_SYNCS` | `10` | Sparse sync rounds between full recovery checkpoints |
